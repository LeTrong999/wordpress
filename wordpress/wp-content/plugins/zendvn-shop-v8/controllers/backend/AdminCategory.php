...
class Zendvn_Sp_AdminCategory_Controller{
	...
	public function __construct(){
		...
		if($zController->getParams('taxonomy') == 'za_category'){
			add_action('za_category_add_form_fields', array($this,'display'));
			add_action('za_category_edit_form_fields', array($this,'display'));
	...
	public function display($term){
		...
		if($action == 'add'){
			$html 		= '<div class="form-field">'
						  ...
						  . ' ' . $btnMedia
						 . $htmlObj->pTag(esc_html__('Upload image for ZS category')) . '</div>';			
			echo $html;
